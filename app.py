"""
MikroPer PDKS API - Template
Yeni müşteri kurulumu için boş iskelet
"""
...
SCHEMA = os.getenv('SCHEMA', 'mikroper')
...
def get_stats():
    ...
    stats = {'total_personel': 0, 'today_present': 0, 'late': 0, 'absent': 0}
    
    # Toplam personel
    result = query_db(f'SELECT COUNT(*) as cnt FROM {SCHEMA}.personel WHERE aktif = 1')
    if result:
        stats['total_personel'] = result[0].get('cnt', 0)
    
    # Bugün gelen
    today = datetime.now().strftime('%Y-%m-%d')
    result = query_db(f'SELECT COUNT(DISTINCT personelid) as cnt FROM {SCHEMA}.hareketler WHERE tarih = %s', (today,))
    if result:
...
def test_connection():
    ...
    if conn:
        try:
            ...
            cursor.execute(f'SELECT COUNT(*) FROM {SCHEMA}.personel')
            personel = cursor.fetchone()[0]
            cursor.close()
            conn.close()
            return jsonify({
                'status': 'connected',
...
def get_personel():
    """Personel listesi"""
    result = query_db(f'''
        SELECT id, sicilno, adi, soyadi, departman, bolum, gorev, aktif
