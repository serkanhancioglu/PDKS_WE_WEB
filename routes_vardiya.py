...
def register_vardiya_routes(app, query_db, SCHEMA):
    ...
    def vardiya_ekle():
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                f'INSERT INTO {SCHEMA}.vardiyalar (vardiyaid, basla, bitis, gunlukhesapid) VALUES (%s, %s, %s, 1) RETURNING id',
                (vardiyaid, basla, bitis)
            )
    ...
    def vardiya_guncelle(vardiya_id):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                f'UPDATE {SCHEMA}.vardiyalar SET basla = %s, bitis = %s WHERE id = %s',
                (basla, bitis, vardiya_id)
            )
            conn.commit()
            cursor.close()
            conn.close()
    ...
    @app.route('/api/vardiya-gruplari', methods=['GET'])
    def get_vardiya_gruplari():
        """Vardiya grupları listesi"""
        result = query_db(f'''
            SELECT id, aciklama, aktif
            FROM {SCHEMA}.vardiyagruplari
            WHERE aktif = 1
            ORDER BY aciklama
        ''')
        return jsonify(result or [])
